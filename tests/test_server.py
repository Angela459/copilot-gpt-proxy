...
from copilot_gpt_proxy.logging import (
    ConsoleLogFormatter,
    TerminalSpinner,
)
...
class CliAndHelperTests(unittest.TestCase):
    ...
    def test_cli_accepts_empty_apply_patch_policy(self) -> None:
        args = build_arg_parser().parse_args(
            ["--empty-apply-patch", "reject", "--max-tool-retries", "0"]
        )
        self.assertEqual(args.empty_apply_patch, "reject")
        self.assertEqual(args.max_tool_retries, 0)
...
class HttpBoundaryTests(unittest.TestCase):
    ...
    def test_normal_logging_summarizes_without_bodies_or_keys(self) -> None:
        with self.assertLogs("copilot_gpt_proxy", level="INFO") as captured:
            status, _ = _post(
                f"{self.proxy.url}/v1/chat/completions",
                self._request(),
                api_key="sk-from-cursor",
