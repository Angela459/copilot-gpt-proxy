...
from .streaming import CopilotReasoningDisplayAdapter, StreamAccumulator
...
class BufferedUpstreamResponse:
    ...
    def __enter__(self) -> "BufferedUpstreamResponse":
        return self
...
class GPTProxyServer(ThreadingHTTPServer):
    config: ProxyConfig
    trace_writer: TraceWriter | None


class GPTProxyHandler(BaseHTTPRequestHandler):
    server_version = "CopilotGPTProxy/0.1"
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if self.config.empty_apply_patch != "allow":
                response, invalid_calls = self._guard_empty_apply_patch(
                    response,
                    prepared.payload,
                    upstream_url,
                    upstream_headers,
                ...
                if invalid_calls:
                    spinner.stop()
                    LOG.error(
                        "blocked repeated empty apply_patch call(s): %s",
                        ", ".join(invalid_calls),
                    )
                    self._send_json(
                        502,
                        {
                            "error": {
                                "message": EMPTY_APPLY_PATCH_ERROR,
                                "type": "empty_apply_patch",
                                "code": "empty_apply_patch",
                    ...
                    self._finish_trace(
                        ...
                        "empty_apply_patch",
                        http_status=502,
                        retries=self._empty_apply_patch_retry_limit(),
                    )
                    return
    ...
    def _guard_empty_apply_patch(
        ...
        upstream_headers: dict[str, str],
        trace: TraceRequest | None,
    ) -> tuple[BufferedUpstreamResponse, list[str]]:
        current_payload = payload
        retry_limit = self._empty_apply_patch_retry_limit()
        for attempt in range(retry_limit + 1):
            buffered, messages = self._buffer_upstream_response(
                response, bool(current_payload.get("stream"))
            )
            invalid_calls = empty_apply_patch_calls(messages)
            if not invalid_calls:
                return buffered, []
            buffered.close()
            LOG.warning(
                "intercepted empty apply_patch call(s) attempt=%s/%s ids=%s",
                attempt + 1,
                retry_limit + 1,
                ",".join(invalid_calls),
            )
            if attempt >= retry_limit:
                return buffered, invalid_calls

            current_payload = repair_request(payload)
            upstream_body = json.dumps(
                current_payload, ensure_ascii=False, separators=(",", ":")
            ).encode("utf-8")
            if trace is not None:
        ...
        raise AssertionError("empty apply_patch retry loop exhausted unexpectedly")

    def _empty_apply_patch_retry_limit(self) -> int:
        if self.config.empty_apply_patch != "retry_once":
            return 0
        return min(1, max(0, self.config.max_tool_retries))

    def _buffer_upstream_response(
        self, response: Any, streaming: bool
    ) -> tuple[BufferedUpstreamResponse, list[dict[str, Any]]]:
        ...
        try:
            if streaming:
                ...
                try:
                    ...
                    completion = None
                messages = messages_from_completion(completion)
        finally:
            response.close()
        return BufferedUpstreamResponse(body, status, headers), messages
    ...
    def _proxy_streaming_response(
        ...
        usage: dict[str, Any] | None = None
        display_adapter = (
            CopilotReasoningDisplayAdapter(self.config.collapsible_reasoning)
        ...
        while True:
            ...
            if not line:
                break
            rewritten, finalized, chunk_usage = self._rewrite_sse_line(
                line,
                original_model,
                accumulator,
                display_adapter,
    ...
    def _rewrite_sse_line(
        ...
        original_model: str,
        accumulator: StreamAccumulator,
        display_adapter: CopilotReasoningDisplayAdapter | None,
        trace: TraceRequest | None = None,
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the local Copilot GPT compatibility proxy"
    )
    parser.add_argument(
        "--config",
        dest="config_path",
    ...
    parser.add_argument(
        "--model",
        help="Fallback model when the request has no model, default from config",
    )
    parser.add_argument(
        "--base-url",
        help="OpenAI-compatible base URL, default from config",
    )
    parser.add_argument(
        "--thinking",
        choices=["enabled", "disabled"],
        help="Thinking mode, default from config or enabled",
    )
    parser.add_argument(
        "--reasoning-effort",
        choices=["low", "medium", "high", "max", "xhigh"],
        help="Reasoning effort, default from config or max",
    )
    parser.add_argument(
        "--verbose",
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Mirror provider reasoning output into Copilot-visible content",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--empty-apply-patch",
        choices=["retry_once", "reject", "allow"],
        help=(
            "How to handle apply_patch without patch content: retry_once "
            "(default), reject immediately, or allow unchanged"
        ),
    )
    parser.add_argument(
        "--max-tool-retries",
        type=int,
        choices=[0, 1],
        help="Maximum compatibility retries for malformed tool calls (0 or 1)",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.empty_apply_patch is not None:
        updates["empty_apply_patch"] = args.empty_apply_patch
    if args.max_tool_retries is not None:
        updates["max_tool_retries"] = args.max_tool_retries
    if updates:
    ...
    if config.trace_dir is not None:
        try:
            ...
            return 2
    server = GPTProxyServer((config.host, config.port), GPTProxyHandler)
